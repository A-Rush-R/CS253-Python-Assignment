...
def dt(train_data_stand, train_labels, test_data_stand) -> pd.DataFrame :

    X_train, X_val, y_train, y_val = train_test_split(train_data_stand, train_labels, test_size = 0.2, random_state = 42, stratify=train_labels)
    ...
    y_test_pred.to_csv("submission_dt.csv", index=False)

    print("Results saved successfully to submission_dt.csv")
